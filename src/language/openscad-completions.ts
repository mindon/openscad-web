...
// Built lazily because `monaco` is a runtime global only available *after*
// loader.init() has resolved. Evaluating these at module top-level used to
// throw "monaco is not defined" and abort the whole entry import chain.
let _builtinCompletions: any[] | null = null;
    ...
    return parsedFiles[path] ??= new Promise(async (res, rej) => {
      ...
      await Promise.all(
        [
          ...
          ...(ownDefs.includes ?? []).map((p) =>
            [p, false] as [string, boolean]
          ),
      ...
      try {
        ...
        if (includeMatch) {
          ...
          const suggestions: Array<
            Partial<monaco.languages.CompletionItem> & {
              label: string;
          ...
          if (!files) {
            ...
            for (const file of files) {
              ...
              if (
                folderPrefix == "" && (file in zipArchives) &&
        ...
        if (statementMatch) {
          ...
          const suggestions: CompletionItem[] = [
            ...
            ...((parsed.vars ?? []).filter((name) => name.indexOf(word) >= 0)
              .map((name) => ({
                label: name,
          ...
          suggestions.sort((a, b) =>
            a.insertText.indexOf(start) - b.insertText.indexOf(start)
          );
          return { suggestions };
        }
        ...
        const suggestions = named.map(([n, s]) =>
          s as any as monaco.languages.CompletionItem
        );
        return { suggestions };
      } catch (e) {
