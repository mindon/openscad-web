...
export async function imageToBlurhash(imageUrl: string): Promise<string> {
  const { rgba, w, h } = await getImageThumbnail(imageUrl, {
    maxSize: 100,
    opaque: true,
  });
  const parts = 9;
...
export async function imageToThumbhash(imagePath: string): Promise<string> {
  const { rgba, w, h } = await getImageThumbnail(imagePath, {
    maxSize: 100,
    opaque: false,
  });
...
export function blurHashToImage(
  hash: string,
  width: number,
  height: number,
): string {
...
export function thumbHashToImage(hash: string): string {
  const { w: width, h: height, rgba } = thumbHashToRGBA(
    new Uint8Array([...atob(hash)].map((c) => c.charCodeAt(0))),
  );
  const canvas = document.createElement("canvas");
  canvas.width = width;
  canvas.height = height;
