// IMPORTANT: use `WebIO` in the browser. `NodeIO` ends up doing
// `await import("fs")` inside @gltf-transform/core's lazy init, which the
// browser can't resolve and throws
//   "TypeError: Failed to resolve module specifier 'fs'"
// at the point we try to encode the OFF -> GLB preview / display file.
import { Accessor, Document, Primitive, WebIO } from "@gltf-transform/core";
import {
  KHRLightsPunctual,
  Light as LightDef,
...
export async function exportGlb(
  ...
  const glb = await new WebIO().registerExtensions([KHRLightsPunctual])
    .writeBinary(doc);
