...
import { IndexedPolyhedron } from "./common.ts";
import { v4 as uuidv4 } from "uuid";
import chroma from "chroma-js";

function getColorMapping(
  colors: chroma.Color[],
  projectedColors: chroma.Color[],
): number[] {
...
export function export3MF(
  ...
  if (extruderColors) {
    ...
    dataColors.forEach((from, i) => {
      ...
      console.log(
        `- ${from.name()} -> ${to?.name()} (${PAINT_COLOR_MAP[extruderIndex]})`,
      );
    });
  }

  const paintColorByColorIndex = extruderIndexByColorIndex?.map((i) =>
    PAINT_COLOR_MAP[i]
  );
  ...
  const archive = {
    "3D/3dmodel.model": new TextEncoder().encode([
      ...
      ...data.colors.map((color, i) =>
        `<base name="color_${i}" displaycolor="${chroma.rgb(...color).hex()}"/>`
      ),
      "</basematerials>",
      ...
      ...data.faces.map((face, i) => {
        ...
        const paintColor = paintColorByColorIndex &&
          paintColorByColorIndex[colorIndex];
        if (paintColor) {
