...
import type { MergedOutputs } from "./openscad-worker.ts";
...
export type OpenSCADInvocationCallback =
  | { result: OpenSCADInvocationResults }
  | ProcessStreams;
...
export function spawnOpenSCAD(
  ...
  return AbortablePromise<OpenSCADInvocationResults>(
    (
      resolve: (result: OpenSCADInvocationResults) => void,
      reject: (error: any) => void,
    ) => {
      const base = () => "/runner/";
      worker = new Worker(base() + "./openscad-worker.ts", { type: "module" });
      rejection = reject;
