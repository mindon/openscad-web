...
export function parseMergedOutputs(
  ...
  const addError = (error: string, _file: string, line: number) => {
    markers.push({
  ...
  const shiftSourceName = opts.shiftSourceLines &&
    opts.shiftSourceLines.sourcePath;
