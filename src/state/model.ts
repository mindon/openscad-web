...
export class Model {
  ...
  init() {
    if (
      !this.state.output && !this.state.lastCheckerRun &&
      !this.state.previewing && !this.state.checkingSyntax &&
  ...
  setVar(name: string, value: any) {
    this.mutate((s) =>
      s.params.vars = { ...s.params.vars ?? {}, [name]: value }
    );
    this.render({ isPreview: true, now: false });
  }

  get logsVisible(): boolean {
    return !!this.state.view.logs;
  }

  set logsVisible(value: boolean) {
    // Pure state write. The logs panel lives in <openscad-app> and mirrors
    // `view.logs` via store subscription — no layout switch needed.
  ...
  changeMultiVisibility(target: MultiLayoutComponentId, visible: boolean) {
    this.mutate((s) => {
      ...
      if (
        (s.view.layout.customizer ? 1 : 0) + (s.view.layout.editor ? 1 : 0) +
            (s.view.layout.viewer ? 1 : 0) == 0
      ) {
    ...
    if (
      this.mutate((s) => {
        if (s.params.activePath != path) {
          // Try to read a file from BrowserFS. Returns `null` when the file
          // isn't present on the virtual filesystem (typical for in-memory
          // sources such as the default `/playground.scad`, or any source
          // that only ever lived in `params.sources`). ENOENT is therefore
          // an expected condition and must NOT be surfaced as a user-facing
          // error — only genuinely unexpected failures get logged.
          const readSource = (p: string): string | null => {
            try {
              return new TextDecoder("utf-8").decode(this.fs.readFileSync(p));
            } catch (e: any) {
              const code = e && (e.code ?? e.errno);
              const isNotFound = code === "ENOENT" || code === 2 ||
                code === "ENOTDIR";
              if (!isNotFound) {
          ...
          // Remove source of previous active path if it's unmodified compared
          // to what's on disk. If the previous file isn't on the FS at all
          // (readSource returns null) we keep its in-memory source as-is so
          // we don't lose the user's edits when they switch away.
          const activePathContent = readSource(s.params.activePath);
          if (activePathContent !== null) {
            s.params.sources = s.params.sources.filter((src) =>
              src.path !== s.params.activePath ||
              src.content != activePathContent
            );
          }

          s.params.activePath = path;
          if (!s.params.sources.find((src) => src.path === path)) {
            // For a brand-new active path that we haven't tracked yet, pull
            // its content off BrowserFS if available. If it's missing we
            // still register an empty entry so downstream code (editor,
            // syntax checker, renderer) can treat it as a known source and
            // populate it later (e.g. via `processSource` / fetchSource).
  ...
  get source(): string {
    return this.state.params.sources.find((src) =>
      src.path === this.state.params.activePath
    )?.content ?? "";
  }
  ...
  private async processSource() {
    let src = this.state.params.sources.find((src) =>
      src.path === this.state.params.activePath
    );
    if (src && src.content == null) {
      ...
      if (url && (match = url.match(githubRx))) {
        url = `https://raw.githubusercontent.com/${match[1]}/${
          match[2]
        }/refs/heads/${match[3]}`;
      }
      const content = new TextDecoder().decode(
        await fetchSource(this.fs, { path, url }),
      );
      this.mutate((s) => {
        s.params.sources = s.params.sources.map((src) =>
    ...
    if (this.source.trim() !== "") {
      ...
      await this.render({ isPreview: true, now: false });
    }
  }
  ...
  async checkSyntax() {
    this.mutate((s) => {
      ...
      // Clear previous errors at the start; they will be re-populated if
      // the check fails.  Syntax errors are reflected in
      // lastCheckerRun.markers (not in s.error).
      s.error = undefined;
    });
    try {
      ...
      this.mutate((s) => {
        s.checkingSyntax = false;
        s.error = `Syntax check failed: ${err}`;
      });
    }
  }
  ...
  async export() {
    ...
    if (
      !this.state.is2D && this.state.params.exportFormat3D == "3mf" &&
    ...
    try {
      ...
      if (exportFormat === "3mf") {
        ...
        output = {
          outFile: new File(
            [exportedData],
            this.state.output.outFile.name.replace(".off", ".3mf"),
          ),
          elapsedMillis,
          logText: "",
          markers: [],
  ...
  async render(
    { isPreview, mountArchives, now, retryInOtherDim }: {
      isPreview: boolean;
    ...
    try {
      ...
      if (
        output.outFile.name.endsWith(".svg") ||
        output.outFile.name.endsWith(".dxf")
      ) {
        is2D = true;
      ...
      // An empty outFile means OpenSCAD produced no top-level geometry
      // (typical for library-only .scad files such as BOSL2's
      // `libtriangles.scad`). Skip the OFF -> GLB conversion to avoid
      // tripping `parseOff('')`/`exportGlb` and surface a clean "no preview"
      // state instead of a red error.
      const isEmptyOutput = output.outFile.size === 0;
      if (!isEmptyOutput && displayFile.name.endsWith(".off")) {
        const offData = parseOff(await displayFile.text());
        const glbBlob = await exportGlb(offData);
        displayFile = new File(
          [glbBlob],
          displayFile.name.replace(".off", ".glb"),
          { type: "model/gltf-binary" },
        );
      }
      const outFileURL = isEmptyOutput
        ? ""
        : URL.createObjectURL(output.outFile);
      const displayFileURL = isEmptyOutput
        ? ""
        : (displayFile && await readFileAsDataURL(displayFile));
      this.mutate((s) => {
        setRendering(s, false);
        // Preserve s.error if it was set by checkSyntax(); only clear it
        // when we're about to show a successful render (s.error is for
        // render failures, not syntax-check failures).
        s.is2D = is2D;
        s.lastCheckerRun = {
    ...
    if (retryInOtherDim) {
      ...
      for (const [pipe, line] of this.state.currentRunLogs ?? []) {
        if (line == "Current top level object is not a 3D object.") {
          ...
          line.includes("WARNING: Mixing 2D and 3D objects is not supported")
        ) {
          isMixed = true;
        }
      }
