...
export async function getBrowserFSLibrariesMounts(
  ...
  const results: [string, ArrayBuffer][] = await Promise.all(
    archiveNames.map(async (
      n: string,
    ) => [n, await fetchData(`${librariesUrl}${n}.zip`)]),
  );
...
export async function symlinkLibraries(
  ...
  await Promise.all(archiveNames.map((n) =>
    (async () => {
      if (!(n in zipArchives)) {
        throw new Error(
          `Archive named ${n} invalid (valid ones: ${
            deployedArchiveNames.join(", ")
          })`,
        );
      }
...
export async function createEditorFS(
  { prefix, allowPersistence, librariesUrl }: {
    prefix: string;
    allowPersistence: boolean;
    // Base URL used to fetch the library ZIP archives. Defaults to
    // "./libraries/" (correct for the worker, which is served from /).
    // The main thread serves from the site root, so it must pass
    // "./libraries/" to reach the deployed archives.
    librariesUrl?: string;
  },
): Promise<FS> {
